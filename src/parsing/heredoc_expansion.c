/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   heredoc_expansion.c                                :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: dgeorgiy <dgeorgiy@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/14 13:52:33 by dgeorgiy          #+#    #+#             */
/*   Updated: 2025/08/15 18:15:19 by dgeorgiy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "parsing.h"
#include "expansions.h"
#include "minishell.h"

int	hd_find_exp_var(t_hd_exp *hd_exp, char **envp)
{
	char	*var;
	char	*exp_var;

	var = ft_calloc(hd_exp->varlen + 1, sizeof(char));
	if (!var)
		return (-42);
	var = ft_memcpy(var, hd_exp->current, hd_exp->varlen);
	exp_var = find_var_in_envp(envp, var);
	free(var);
	if (!exp_var)
		hd_exp->exp_var = NULL;
	else
	{
		hd_exp->exp_var = ft_strdup(exp_var);
		if (!hd_exp->exp_var)
			return (-42);
	}
	return (0);
}

void	expand_var(t_hd_exp *hd_exp, char **envp)
{
	hd_exp->varlen = find_varlen(hd_exp->current);
	if (hd_exp->varlen == 0)
	{
		hd_exp->i++;
		hd_exp->current++;
		return ;
	}
	hd_exp->i += hd_exp->varlen;
	hd_find_exp_var(hd_exp, envp);
	if (hd_exp->exp_var)
	{
		hd_exp->temp = hd_exp->str;
		hd_exp->str = ft_strjoin(hd_exp->temp, hd_exp->exp_var);
		if (!(hd_exp->str))
			return (free_hd_exp(hd_exp), exit(-42));
		(free(hd_exp->temp), free(hd_exp->exp_var));
		hd_exp->temp = NULL;
		hd_exp->exp_var = NULL;
		hd_exp->strlen += hd_exp->exp_varlen;
	}
	hd_exp->current += hd_exp->varlen;
}

void	expand_hd_code(t_hd_exp *hd_exp, t_cshell *cshell)
{
	if (*(hd_exp->current) == '$')
		hd_exp->exp_var = ft_itoa(cshell->shell_id);
	else
		hd_exp->exp_var = ft_itoa(cshell->exit_code);
	hd_exp->exp_varlen = ft_strlen(hd_exp->exp_var);
	hd_exp->temp = hd_exp->str;
	hd_exp->str = ft_strjoin(hd_exp->temp, hd_exp->exp_var);
	(free(hd_exp->temp), free(hd_exp->exp_var));
	hd_exp->temp = NULL;
	hd_exp->exp_var = NULL;
	hd_exp->strlen += hd_exp->exp_varlen;
	hd_exp->i++;
	hd_exp->current++;
}

void	expand_dollar_sign(t_hd_exp *hd_exp, t_cshell *cshell)
{
	char	c;

	hd_exp->i++;
	hd_exp->current++;
	c = *(hd_exp->current);
	if (c == ' ' || c == '\0' || c == '\'' || c == '"')
	{
		hd_exp->temp = hd_exp->str;
		hd_exp->str = ft_strjoin(hd_exp->temp, "$");
		if (!(hd_exp->str))
			return (free_hd_exp(hd_exp), exit(-42));
		hd_exp->temp = NULL;
		hd_exp->strlen++;
		return ;
	}
	if (c == '$' || c == '?')
		return (expand_hd_code(hd_exp, cshell));
	expand_var(hd_exp, cshell->envp);
}

char	*heredoc_expand(char *str, t_cshell *cshell)
{
	t_hd_exp	hd_exp;
	char		*expanded_string;

	init_hd_exp(&hd_exp, str);
	while ((hd_exp.og_string)[hd_exp.i])
	{
		if (*(hd_exp.current) == '$')
			expand_dollar_sign(&hd_exp, cshell);
		else
			add_one_char_to_hd_str(&hd_exp);
	}
	expanded_string = ft_strdup(hd_exp.str);
	free(str);
	str = NULL;
	free_hd_exp(&hd_exp);
	return (expanded_string);
}
